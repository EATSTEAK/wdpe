    ...
    {   $(#[$outer:meta])*
        ...
        impl<'a> $crate::element::Element<'a> for $name<'a> {
            ...
            fn from_ref(
                ...
                Ok(Self::new($crate::element::definition::ElementDefinition::id_cow(element_def), element))
            }
...
macro_rules! define_element_interactable {
    {
        ...
        $crate::element::macros::define_element_base!{
            $(#[$outer])*
            $name<$controlid, $element_name> {
                lsevents: std::cell::OnceCell<Option<$crate::element::EventParameterMap>>,
                $($sfield : $stype, )*
            },
            $(#[$def_outer])*
            $def_name,
